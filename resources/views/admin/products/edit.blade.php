...
@section('content')
<div class="card">
    <div class="card-header">Product - Edit <a href="{{ route('products.index') }}" class="btn btn-light float-right btn-sm "><i class="fas fa-chevron-left"></i> Go Back</a></div>

    <div class="card-body">
        <form method="post" action="{{ route('products.update', $blog->id) }}" enctype="multipart/form-data" novalidate>
            @csrf
            {{ method_field('PUT') }}
            <div class="row">
                <div class="col-md-8">
                ...
                <div class="col-md-4">
                    ...
                    <div class="form-group">
                        <label for="image">Multiple Images (optional)</label>
                        <div style="width:100%">
                            @foreach($multi_images as $multi)
                            <img width="100" class="img-fluid" src="{{asset('storage/'.$multi->thumbnail.'')}}" alt="{{ $blog->title }}">
                            <span><a href="{{ route('products.deletemulti', $multi->id) }}">delete</a></span>
                            @endforeach
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="categories" name="categories[]" required multiple>
                            @if (is_array(old('pcategories')))
                                @foreach (old('pcategories') as $oldCategory)
                                ...
                                @endforeach
                            @else
                                @foreach($blog->pcategories as $category)
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="subcategories" name="subcategories[]"  multiple>
                            @if (is_array(old('subpcategories')))
                                @foreach (old('subpcategories') as $oldCategory)
                                <option value="{{ $oldCategory }}" selected="selected">
                                    @if(is_numeric($oldCategory))
                                        {{ App\Subpcategory::where('id', $oldCategory)->get()->pluck('name')[0] }}
                                    @else
                                        {{ $oldCategory }}
                                    @endif
                                </option>
                                @endforeach
                            @else
                                @foreach($blog->subpcategories as $category)
                                <option value="{{ $category->id }}" selected>{{ $category->name }}</option>
                                @endforeach
                            @endif
                        </select>
                    ...
                    <div class="form-group">
                        <label style="width: 100%;" for="is_active">Tags</label>
                        <input style="width: 100%;" value="{{ old('tags', $blog->tags) }}"  class="form-control" placeholder="Tags" data-role="tagsinput" type="text" name="tags" >
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="meta_title" name="meta_title" value="{{ old('meta_title', $blog->meta_title) }}" placeholder="Meta title" onkeyup="countChar(this, 'charNumTitlemeta', 125)" maxlength="125">
                        <span id="charNumTitlemeta" class="text-info">125 Characters Left</span>
                    </div>

                    <div class="form-group">
                        <label for="is_active">Meta Keywords</label>
                        <textarea name="meta_keywords" id="meta_keywords" class="form-control" rows="3" maxlength="280" placeholder="Meta Keywords" onkeyup="countChar(this, 'charNumExcerptKey', 280)">{{ old('meta_keywords', $blog->meta_keywords) }}</textarea>
                        <span id="charNumExcerptKey" class="text-info">280 Characters Left</span>
                    </div>

                    <div class="form-group">
                        <label for="is_active">Meta Description</label>
                        <textarea name="meta_description" id="meta_description" class="form-control" rows="3" maxlength="280" placeholder="Meta Description" onkeyup="countChar(this, 'charNumExcerptDescription', 280)">{{ old('meta_description', $blog->meta_description) }}</textarea>
                        <span id="charNumExcerptDescription" class="text-info">280 Characters Left</span>
                    </div>

                    <div class="form-group">
                        <label for="is_active">New Launches <span class="required">*</span></label>
                        <select class="form-control" id="is_active" name="new_launches" required>
                            <option value="1" @if(old('new_launches', $blog->new_launches) == 1) selected @endif>Yes</option>
                            <option value="0" @if(old('new_launches', $blog->new_launches) == 0) selected @endif>No</option>
                        </select>
                    </div>

                    <div class="form-group">
                        <label for="is_active">Hot Offers <span class="required">*</span></label>
                         <select class="form-control" id="is_active" name="hot_offers" required>
                            <option value="1" @if(old('hot_offers', $blog->hot_offers) == 1) selected @endif>Yes</option>
                            <option value="0" @if(old('hot_offers', $blog->hot_offers) == 0) selected @endif>No</option>
                        </select>
                    </div>
...
$(document).ready(function() {
    $('#categories').select2({
        ...
        tokenSeparators: [',','.'],
        ajax: {
            url: '{{ route("catlist.ajaxSelectData") }}',
            dataType: 'json',
            cache: true,
    ...
    $('#subcategories').select2({
        ...
        tokenSeparators: [',','.'],
        ajax: {
            url: '{{ route("subcatlist.ajaxSelectData") }}',
            dataType: 'json',
            cache: true,
            data: function(params) {
              return {
