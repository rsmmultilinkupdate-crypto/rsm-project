...
<div class="card">
    ...
    <div class="card-body">
        <form method="post" action="{{ route('subproductcategories.update', $category->id) }}" enctype="multipart/form-data" novalidate>
            @csrf
            {{ method_field('PUT') }}
            <div class="row">
                <div class="col-md-5">
                    ...
                    <div class="form-group">
                        <label for="categories">Categories {{ $category->pcategories_id }}<span class="required">*</span></label>
                        <select class="form-control" id="categories" name="categories" required >
                            @foreach ($categories as $oldCategory)
                            <option <?php if($oldCategory->id == $category->pcategories_id){ echo 'selected="selected"'; } ?> value="{{ $oldCategory->id }}" >
                                    {{ $oldCategory->name }}
                            </option>
                            @endforeach
                        </select>
